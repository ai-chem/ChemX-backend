...
async def get_seltox_column_stats(
    db: Session = Depends(get_db),
    file_format: str = Query("json", enum=["json", "csv"]),
) -> Response:
    try:
...
async def get_seltox_row_stats(
    db: Session = Depends(get_db),
    file_format: str = Query("json", enum=["json", "csv"]),
) -> Response:
    try:
...
async def get_seltox_top_categories(
    db: Session = Depends(get_db),
    file_format: str = Query("json", enum=["json", "csv"]),
) -> Response:
    try:
...
async def get_seltox_ml_data(
    db: Session = Depends(get_db),
    file_format: str = Query("json", enum=["json", "csv"]),
) -> Response:
    try:
        ml_data = SeltoxService.get_ml_data(db)
        return create_downloadable_response(
            data=ml_data, file_format=file_format, base_filename="seltox_ml_data"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
